const App = {
    ...
    showLogin() {
        ...
        document.getElementById('root').innerHTML = this.getLoginHTML();
    },
    ...
    getLoginHTML() {
        const code = localStorage.getItem('brainrotflip_verification_code') || this.generateVerificationCode();
        return `
            <div class="modal-overlay">
                <div class="modal">
                    ...
                    <div class="verification-box">
                        <label>Put this code in your Roblox description:</label>
                        <div class="verification-code">
                            <code>${code}</code>
                            <button class="copy-btn" onclick="App.copyVerificationCode('${code}')">Copy</button>
                        </div>
                    </div>
                    <div class="cf-turnstile" data-sitekey="0x4AAAAAAAzC9w_G_tXXXXXX" data-theme="dark"></div>
                    <button class="modal-btn" onclick="App.verifyAndLogin()">Verify & Login</button>
                    <p class="modal-note">Complete verification then login</p>
                </div>
            </div>
        `;
    },
    ...
    showApp() {
        document.getElementById('root').innerHTML = this.getAppHTML();
        this.updateUI();
        this.loadCoinflips();
        this.loadChat();
        this.startIntervals();
                    ...
                    <header class="header">
                        ...
                        <div class="header-right">
                            <div class="level-badge">
                                <span class="level-number" id="headerLevel">1</span>
                                <span>Level</span>
                            </div>
    ...
    navigateTo(page) {
        ...
        else if (page === 'coinflip') content.innerHTML = this.getCoinflipHTML();
        else if (page === 'profile') content.innerHTML = this.getProfileHTML();
        else if (page === 'leaderboard') content.innerHTML = this.getLeaderboardHTML();
        else if (page === 'admin') content.innerHTML = this.getAdminHTML();

        document.querySelectorAll('.nav-item').forEach(n => n.classList.remove('active'));
        const targetNav = Array.from(document.querySelectorAll('.nav-item')).find(n => 
            n.textContent.trim().toLowerCase().includes(page)
        );
        if (targetNav) targetNav.classList.add('active');

        if (page === 'coinflip') this.loadCoinflips();
        if (page === 'profile') {
    ...
    getCoinflipHTML() {
        return `
            <div class="page active">
                ...
                <div class="stats-row">
                    <div class="stat-card">
                        <div class="value" id="cfStatRooms">0</div>
                        <div class="label">Rooms</div>
                    </div>
                    <div class="stat-card">
                        <div class="value" id="cfStatValue">0</div>
                        <div class="label">Value</div>
                    </div>
                    <div class="stat-card">
                        <div class="value" id="cfStatItems">0</div>
                ...
                <div class="filters-row">
                    <div class="filter-group">
                        <span>🟢</span>
                        <span id="filterHeadsCount">0</span>
                    </div>
                    <div class="filter-group">
                        <span>🔴</span>
                        <span id="filterTailsCount">0</span>
                    </div>
                </div>
            ...
            <div class="page active">
                ...
                <div class="profile-stats-grid">
                    <div class="profile-stat-card">
                        <div class="icon">💎</div>
                        <div class="value" id="profileWagered">0</div>
                        <div class="label">Wagered</div>
                    </div>
                    <div class="profile-stat-card">
                        <div class="icon">🏆</div>
                        <div class="value" id="profileWon">0</div>
                        <div class="label">Won</div>
                    </div>
                    <div class="profile-stat-card">
                        <div class="icon">📉</div>
                        <div class="value" id="profileLost">0</div>
    ...
    updateUI() {
        const balance = DB.getUserBalance(this.currentUser.username);
    ...
    loadCoinflips() {
        const coinflips = DB.getActiveCoinflips();
        ...
        if (valueEl) valueEl.textContent = this.formatNumber(coinflips.reduce((s, cf) => s + cf.totalValue, 0));
        if (itemsEl) itemsEl.textContent = coinflips.reduce((s, cf) => s + cf.creatorItems.length, 0);

        const headsCount = coinflips.filter(cf => cf.creatorSide === 'H').length;
        ...
        container.innerHTML = coinflips.map(cf => `
            <div class="coinflip-card">
                <div class="cf-players">
                    <div class="cf-player ${cf.creatorSide === 'H' ? 'green' : 'red'}">
                        <img class="cf-player-avatar" src="${cf.creatorAvatar}">
                        <div class="cf-player-info">
                            <div class="name ${cf.creatorSide === 'H' ? 'green' : 'red'}">${cf.creator}</div>
                            <div style="font-size:0.8rem;color:var(--text-secondary);">${cf.creatorItems.length} items</div>
                        </div>
                    </div>
                ...
                <div class="cf-value">
                    <div class="amount">${this.formatNumber(cf.totalValue)} 💎</div>
                    <div class="label">Value</div>
                </div>
                ${cf.creator !== this.currentUser.username 
                    ? `<button class="join-btn" onclick="App.openJoinModal('${cf.id}')">Join</button>`
                    : `<span style="color:var(--text-secondary);font-style:italic;">Your game</span>`
                }
            </div>
        `).join('');
    },
    ...
    openCreateModal() {
        ...
        modal.innerHTML = `
            <div class="modal wide">
                ...
                <div class="items-grid" id="createItemsGrid">
                    ${this.currentUser.inventory.length === 0 
                        ? '<p style="color:var(--text-secondary);grid-column:span 4;text-align:center;padding:2rem;">No items</p>'
                        : this.currentUser.inventory.map(item => `
                            <div class="item-card" data-id="${item.uniqueId}" data-value="${item.value}" onclick="App.toggleItem(this,'create')">
                                <div class="icon">${item.icon}</div>
                                <div class="name">${item.name}</div>
                                <div class="value">${item.value} 💎</div>
                            </div>
    ...
    openJoinModal(coinflipId) {
        this.selectedItems = [];
        this.joiningCoinflipId = coinflipId;
        this.currentUser = DB.getUser(this.currentUser.username);

        const cf = DB.getCoinflip(coinflipId);
        if (!cf) return;
        ...
        modal.innerHTML = `
            <div class="modal wide">
                ...
                <p>Joining: <strong>${cf.creator}</strong></p>
                <p style="color:var(--text-secondary);margin-bottom:1rem;">Required: <strong style="color:var(--accent-green);">${this.formatNumber(cf.totalValue)} 💎</strong></p>
                <div class="items-grid" id="joinItemsGrid">
                    ${this.currentUser.inventory.length === 0 
                        ? '<p style="color:var(--text-secondary);grid-column:span 4;text-align:center;padding:2rem;">No items</p>'
                        : this.currentUser.inventory.map(item => `
                            <div class="item-card" data-id="${item.uniqueId}" data-value="${item.value}" onclick="App.toggleItem(this,'join')">
                                <div class="icon">${item.icon}</div>
                                <div class="name">${item.name}</div>
                                <div class="value">${item.value} 💎</div>
                            </div>
    ...
    toggleItem(el, modalType) {
        ...
        if (el.classList.contains('selected')) {
            ...
            const item = this.currentUser.inventory.find(i => i.uniqueId === uniqueId);
            if (item) this.selectedItems.push(item);
        }

        const total = this.selectedItems.reduce((s, i) => s + i.value, 0);
    ...
    selectSide(side, el) {
        ...
        document.querySelectorAll('.side-option').forEach(opt => {
            opt.classList.remove('selected', 'green', 'red');
            if (opt === el) {
                opt.classList.add('selected', side === 'H' ? 'green' : 'red');
            }
        });
    },
    ...
    createCoinflip() {
        ...
        const cf = DB.createCoinflip(this.currentUser.username, this.selectedItems, this.selectedSide);
        if (cf) {
    ...
    confirmJoinCoinflip() {
        const cf = DB.getCoinflip(this.joiningCoinflipId);
        if (!cf) return;

        const selectedTotal = this.selectedItems.reduce((s, i) => s + i.value, 0);
        const minReq = cf.totalValue * 0.9;
        const maxReq = cf.totalValue * 1.1;
        ...
        const joined = DB.joinCoinflip(this.joiningCoinflipId, this.currentUser.username, this.selectedItems);
        if (joined) {
    ...
    startCoinflipAnimation(cf) {
                ...
                <div class="cf-arena">
                    <div class="arena-players">
                        <div class="arena-player">
                            <img src="${cf.creatorAvatar}">
                            <div class="name">${cf.creator}</div>
                            <div class="value">${this.formatNumber(cf.totalValue)} 💎</div>
                        </div>
                        <div class="vs-badge">VS</div>
                        <div class="arena-player red">
                            <img src="${cf.opponentAvatar}">
                            <div class="name">${cf.opponent}</div>
                            <div class="value">${this.formatNumber(cf.opponentItems.reduce((s, i) => s + i.value, 0))} 💎</div>
                        </div>
                    </div>
        ...
        video.onended = () => {
            const result = DB.finishCoinflip(cf.id, winnerSide);
            const banner = document.getElementById('winnerBanner');
            const winnerName = document.getElementById('winnerName');
            
            if (winnerName) winnerName.textContent = result.winner;
            
            const isWinner = result.winner === this.currentUser.username;
            ...
            setTimeout(() => this.closeModal('cfAnimationModal'), 3000);
        };
    },
            ...
            <div class="chat-message">
                ...
                <div class="message-content">
                    <div class="message-header">
                        <span class="message-name ${m.isAdmin ? 'admin' : ''}">${m.username}</span>
    ...
    loadLeaderboard() {
        const users = DB.getLeaderboard('wagered');
        const container = document.getElementById('leaderboardList');
        if (!container) return;

        container.innerHTML = users.map((u, i) => `
            <div class="lb-item">
                <div class="lb-rank ${i === 0 ? 'gold' : i === 1 ? 'silver' : i === 2 ? 'bronze' : ''}">
                    ${i < 3 ? ['🥇', '🥈', '🥉'][i] : '#' + (i + 1)}
                </div>
                <div class="lb-player">
                    <img src="${u.avatar}">
                ...
                <div class="lb-wagered">${this.formatNumber(u.stats.wagered)} 💎</div>
            </div>
        `).join('');
    },
    ...
    loadAdminPanel() {
        ...
        if (select) {
            select.innerHTML = DB.getAllItems().map(i => 
        ...
        if (usersList) {
            ...
            usersList.innerHTML = users.map(u => `
                <div style="display:flex;align-items:center;justify-content:space-between;padding:0.875rem;border-bottom:1px solid var(--border-color);">
                    <div style="display:flex;align-items:center;gap:0.75rem;">
                        <img src="${u.avatar}" style="width:38px;height:38px;border-radius:50%;">
                        <div>
                            <div style="font-weight:600;">${u.username}</div>
                            <div style="font-size:0.8rem;color:var(--text-secondary);">
                                Balance: ${this.formatNumber(DB.getUserBalance(u.username))} 💎
                            </div>
                        </div>
                    </div>
                </div>
